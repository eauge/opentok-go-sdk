...
type Session struct {
	...
	// The OpenTok project data to which the session belongs to
	project OpenTok
...
func (s *Session) StartArchive(name string) (archive Archive, err error) {
	...
	var response []byte
	url := fmt.Sprintf("v2/partner/%d/archive", s.project.ApiKey)
...
func (s *Session) StopArchive(archiveId string) (a Archive, err error) {
	...
	var response []byte
	url := fmt.Sprintf("v2/partner/%d/archive/%s/stop", s.project.ApiKey, archiveId)
	headers := map[string]string{"Content-type": "application/json"}
...
func (s *Session) buildTokenString(dataString string) Token {
	signature := encodeHMAC(dataString, s.project.ApiSecret)
	buffer := bytes.NewBufferString("")
	buffer.WriteString(fmt.Sprintf("partner_id=%d", s.project.ApiKey))
