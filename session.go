...
func (s *Session) StartArchive(name string) (archive Archive, err error) {
	...
	if response, err = s.client.post(url, headers, data); err != nil {
		return Archive{}, err
	}

	if archive, err = decodeArchive(response); err != nil {
...
func (s *Session) StopArchive(archiveId string) (a Archive, err error) {
	...
	if response, err = s.client.post(url, headers, nil); err != nil {
		return Archive{}, err
	}
	if a, err = decodeArchive(response); err != nil {
		return Archive{}, errors.New(fmt.Sprintf("Archive could not be decoded successfully"))
	}
	return a, nil
}
...
func (s *Session) createSession() (response []byte, err error) {
	...
	if response, err = s.client.post("session/create", headers, data); err != nil {
		return nil, err
	}
	return response, nil
}
...
func (s *Session) allocateSession() (id string, err error) {
	var response []byte
	if response, err = s.createSession(); err != nil {
		return "", err
	}

	if id, err = readAllocateResponse(response); err != nil {
		return "", err
	}
	return id, nil
}
